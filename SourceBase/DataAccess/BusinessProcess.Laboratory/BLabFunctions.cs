...
namespace BusinessProcess.Laboratory
{
    public class BLabFunctions : ProcessBase, ILabFunctions
    {
        ...
        public DataTable GetPreviousOrderedLabs(int PatientId)
        {
            lock (this)
            {
                ClsUtility.Init_Hashtable();
                ClsUtility.AddParameters("@PatientId", SqlDbType.VarChar, PatientId.ToString());
                ClsObject UserManager = new ClsObject();
                return (DataTable)UserManager.ReturnObject(ClsUtility.theParams, "pr_Clinical_GetPatientLabOrderHistory", ClsDBUtility.ObjectEnum.DataTable);
            }
        }
