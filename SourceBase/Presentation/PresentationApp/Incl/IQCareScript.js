...
function NotifyMessage(Msg) {
    var objlbl = document.getElementById('ctl00_lblError');
    //alert(Msg);
    if (objlbl != null) {
        var len = objlbl.innerHTML.toString().length;
        if (len > 0) {
            objlbl.innerHTML = '';
        }
        else {            
            objlbl.innerHTML = Msg.toString();
        }
    }
}
...
function validate(InitEval) {
    if (document.InitEval.physKarnofskyScore.value.length > 3 || document.InitEval.physKarnofskyScore.value > 100 || document.InitEval.physKarnofskyScore.value < 1) {
        NotifyMessage("Please enter a score between 1 and 100.");
...
function flagAbnormalValue(obj, Disp, lo, hi) {
    ...
    if ((val < lo) || (val > hi)) {
        ...
        NotifyMessage("Normal range should be in the range of " + lo + " and " + hi);
...
function isBetween(obj, Disp, lo, hi) {
    ...
    if ((val < lo) || (val > hi)) {
        ...
        NotifyMessage(Disp + " should be in the range of " + lo + " and " + hi)
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return (false);
...
function checkMin(obj, Disp, lo) {
    ...
    if (val < lo) {
        ...
        NotifyMessage(Disp + " should be greater then " + lo)
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return (false);
...
function checkMax(obj, Disp, hi) {
    ...
    if (val > hi) {
        ...
        NotifyMessage(Disp + " should be less then " + hi)
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return (false);
...
function check_form(form_name) {
    if (submitted == true) {
        NotifyMessage("This form has already been submitted. Please press Ok and wait for this process to be completed.");
        return false;
    }
    error = false;
    form = form_name;
...
function isCheckValidDateHIVrelated(frmvisitdate, frmotherdate, Disp, obj) {
    ...
    if (frmvisitdatetxt == "") {
        NotifyMessage("Please Enter Visit Date First");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
    ...
    if (frmothertxt <= frmvisitdatetxt) {
        ...
        NotifyMessage(Disp + " date should be before or equal to visit date");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
...
function isCheckValidDate_MMM_YR(frmvisitdate, frmotherdate, Disp, obj) {
    ...
    if (frmothertxt <= frmvisitdatetxt) {
        ...
        NotifyMessage(Disp + " date should be before or equal to the Month & Year of visit date");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
...
function isCheckValidDate_MMM_YR_TBRxSTOP(frmvisitdate, frmotherdate, Disp, obj) {
    ...
    var frmvisitdatetxt = document.getElementById(frmvisitdate).value;
    if (frmvisitdatetxt == "") {
        NotifyMessage("Please Enter Visit TB Rx Start");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
    ...
    if (frmothertxt >= frmvisitdatetxt) {
        ...
        NotifyMessage(Disp + " date should be greater or equal to the Month & Year of TB Rx Start");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
...
function isCheckValidDate_MMM_YR_TBRxStart(frmvisitdate, frmotherdate, Disp, obj) {
    ...
    if (frmothertxt >= frmvisitdatetxt) {
        ...
        NotifyMessage(Disp + " date should be geater than or equal  to the Month & Year of DOB");
        objtxt.value = "";
        objtxt.focus();
        objtxt.select();
        return false;
...
function rdoSelectedYes(rbl, divID) {
...
function fnUncheckallVitals(strcblcontrolId) {
